...
{
    ros::init(argc, argv, "position_planner");
    ros::NodeHandle nh;
    ...
    //航点数
    int flightPoint_num = 5;
    //用来记录当前执行到哪一个航点
    int current_flightPoint = 0;
    //飞行航点位置数组集合,每一行依次存放x, y, z坐标
    double Flight_list[N][3] = {{start_x, start_y, 1.5}, {2, -0.6, 1.5}, {2, -3.1, 1.5}, {0, -3.1, 1.5}, {start_x, start_y, 1.5}};
    ...
    while(ros::ok()){
        ...
        //航点控制逻辑
        if(pose_match(Flight_list[current_flightPoint][0], cur_x, 0.03)&&
           pose_match(Flight_list[current_flightPoint][1], cur_y, 0.03)&&
